...
int main(int argc, char * argv[]) {
    ...
    int64_t s;
    unsigned int range = 1500, fullrange = 2048;
    BiomeFilter filter;
    int minscale;
    ...
    fp_read = fopen("./dictionary/converted.txt", "r");
    fp_write = fopen("./dictionary/found.csv", "w");

    if (fp_read == NULL) return 1;
    if (fp_write == NULL) return 1;

    while ((read = getline(&line,&len, fp_read)) != -1) {
        scanned = sscanf(strtok(line, "\n"), "%"
            SCNd64 "%c",&s,&c);
        if (scanned != 1) continue; // not a number

        if (!checkForBiomes(&g, cache, s, ax, az, w, h, filter, minscale)) continue; // seed did not pass initial biome check
        ...
        if (!huts_found) continue; // seed did not pass witch hut check
        ...
        if (monument_count == 0) continue; // seed did not pass monument check
